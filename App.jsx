...
function BookView({salon,addBooking,onBack,inline,setView,customer,rescheduleId}){
  ...
  const checkConflict=(slM,newDur,bookings,barberId)=>bookings.filter(b=>{if(["rejected","cancelled"].includes(b.status))return false;const bId=b.barberId||b.barber_id;if(barberId&&barberId!=="any"&&bId!==barberId&&bId!=="any")return false;const bM=toM(b.time||"00:00");return slM<bM+getExistingDur(b)+BMIN&&bM<slM+newDur;}).length;
