  ...
  const addBooking=async(sid,bk,rescheduleOldId=null)=>{
    try{
      ...
      if(newBooking.id&&!isReschedule){
        ...
        const nowBks=await sb("bookings","GET",null,`?salon_id=eq.${sid}&date=eq.${bk.date}&status=neq.rejected&select=id,barber_id,time,slot_duration_minutes,created_at`).catch(()=>[]);
...
function BookView({salon,addBooking,onBack,inline,setView,customer,rescheduleId}){
  ...
  const slotStatus=sl=>{if(loadingBookings)return"loading";const slM=toM(sl);const newDur=totalDuration||(salon.slotMin||SLOT_MIN);const matches=liveBookings.filter(b=>{if(b.date!==form.date)return false;if(["rejected","cancelled"].includes(b.status))return false;if(form.barberId&&b.barberId!==form.barberId&&b.barberId!=="any")return false;const bM=toM(b.time||"00:00");return slM<bM+getBookingDur(b)+BMIN&&bM<slM+newDur+BMIN;});if(matches.length<(form.barberId?1:bc))return"free";const direct=matches.some(b=>{const bM=toM(b.time||"00:00");return bM<=slM&&slM<bM+getBookingDur(b);});return direct?"booked":"tight";};
  ...
  const v4=()=>{const e={};if(form.time&&slotFull(form.time)){setForm(p=>({...p,time:""}));e.time=t("book.err_time");}else if(!form.time&&!form.waitSlot){e.time=t("book.err_time");}setErrors(e);return!Object.keys(e).length;};
  ...
  const inner=(
    <>
      ...
      {step===4&&<div style={G.fc}>
        ...
        {loadingBookings?<div style={{textAlign:"center",padding:"18px 0",color:"var(--text-muted)",fontSize:13}}>⏳ جاري التحقق من المواعيد...</div>:<div style={G.timeGrid}>{slotsVisible.map(sl=>{const st=slotStatus(sl);const full=st!=="free";const tight=st==="tight";const sel=form.time===sl;const waiting=form.waitSlot===sl;return(<div key={sl} style={{display:"flex",flexDirection:"column",gap:2}}><button disabled={loadingBookings||(full&&!waiting)} onClick={()=>!full&&setForm(p=>({...p,time:sl,waitSlot:""}))} style={{...G.ts,...(st==="booked"?G.tsF:{}),...(tight?G.tsTight:{}),...(sel&&!full?G.tsS:{})}}><div>{to12h(sl)}</div>{st==="booked"&&<div style={{fontSize:9,marginTop:1,color:"var(--text-muted)"}}>{t("book.booked")}</div>}{tight&&<div style={{fontSize:9,marginTop:1,color:"#e67e22"}}>الوقت لا يكفي</div>}</button>{full&&<button onClick={()=>setForm(p=>({...p,waitSlot:p.waitSlot===sl?"":sl,time:""}))} style={{fontSize:9,padding:"3px 4px",borderRadius:6,border:`1.5px solid ${waiting?"var(--p)":"#f39c1266"}`,background:waiting?"var(--pa12)":"rgba(243,156,18,.06)",color:waiting?"var(--p)":"#f39c12",cursor:"pointer",fontFamily:"inherit",fontWeight:waiting?700:400}}>⏳{waiting?" ✓":""}</button>}</div>);})}</div>}
        ...
        <div style={{display:"flex",gap:8,marginTop:8}}>
          ...
          <button disabled={booking} style={{flex:1,border:"none",padding:"12px",borderRadius:10,fontWeight:700,fontSize:14,cursor:booking?"not-allowed":"pointer",fontFamily:"'Cairo',sans-serif",background:form.waitSlot?"linear-gradient(135deg,#f39c12,#e67e22)":rescheduleId?"linear-gradient(135deg,#8e44ad,#9b59b6)":"linear-gradient(135deg,#27ae60,#2ecc71)",color:"#fff",opacity:booking?0.6:1}} onClick={async()=>{
            ...
            if(form.waitSlot){
              ...
              const freshBks=await sb("bookings","GET",null,`?salon_id=eq.${salon.id}&date=eq.${form.date}&status=neq.rejected&select=id,barber_id,service,time,slot_duration_minutes`).catch(()=>[]);
              const conflict=(Array.isArray(freshBks)?freshBks:[]).filter(b=>{
                ...
                const bM=toM(b.time||"00:00");
                const bDur=b.slot_duration_minutes||(()=>{const bSvcs=(()=>{try{return JSON.parse(b.service||"[]");}catch{return[];}})();const bBarberObj=salon.barbers?.find(x=>x.id===b.barber_id);return bSvcs.reduce((a,s)=>a+((bBarberObj?.durations?.[s])||salonDurations[s]||0),0)||(salon.slotMin||SLOT_MIN);})();
...
const G={
  ...
  tsTight:{background:"rgba(230,126,34,.07)",borderColor:"rgba(230,126,34,.3)",color:"#e67e22",opacity:.75,cursor:"not-allowed"},
