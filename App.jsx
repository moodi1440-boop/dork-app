...
{id:4,name:“روان للحلاقة الرجالية”,owner:“نواف الشمري”,ownerPhone:“0512345678”,region:“منطقة الرياض”,gov:“محافظة الرياض”,center:“مركز الرياض”,village:“النزهة”,phone:“0512345678”,address:“شارع التحلية، حي النزهة”,locationUrl:“https://maps.google.com/?q=24.7136,46.6753”,services:[“قص شعر”,“حلاقة لحية”,“تسريح”,“عناية بالبشرة”,“ماسك وجه”],prices:{“قص شعر”:55,“حلاقة لحية”:35,“تسريح”:45,“عناية بالبشرة”:90,“ماسك وجه”:70},shiftEnabled:true,shift1Start:“09:00”,shift1End:“13:30”,shift2Start:“17:00”,shift2End:“24:00”,workStart:“09:00”,workEnd:“24:00”,barbers:[{id:“b7”,name:“أبو فيصل”},{id:“b8”,name:“راشد”},{id:“b9”,name:“سعود”}],tone:“cash”,bookings:[{id:105,salonId:4,name:“فيصل الحربي”,phone:“0533221144”,services:[“قص شعر”,“حلاقة لحية”],barberId:“b7”,barberName:“أبو فيصل”,date:“2025-04-26”,time:“09:40”,total:90,status:“approved”},{id:106,salonId:4,name:“بدر العنزي”,phone:“0544332211”,services:[“تسريح”,“ماسك وجه”],barberId:“b8”,barberName:“راشد”,date:“2025-04-26”,time:“18:00”,total:115,status:“pending”}],rating:4.9,status:“approved”},
{id:5,name:“الفارس بارشوب”,owner:“عمر البقمي”,ownerPhone:“0545678901”,region:“منطقة مكة المكرمة”,gov:“محافظة مكة المكرمة”,center:“مركز مكة المكرمة”,village:“العزيزية”,phone:“0545678901”,address:“شارع أجياد، العزيزية”,locationUrl:“https://maps.google.com/?q=21.3891,39.8579”,services:[“قص شعر”,“حلاقة كاملة”,“صبغة شعر”,“تنظيف بشرة”],prices:{“قص شعر”:40,“حلاقة كاملة”:70,“صبغة شعر”:120,“تنظيف بشرة”:80},shiftEnabled:false,workStart:“08:30”,workEnd:“23:30”,barbers:[{id:“b10”,name:“أبو عمر”},{id:“b11”,name:“جمال”}],tone:“classic”,bookings:[{id:107,salonId:5,name:“وليد المالكي”,phone:“0566554433”,services:[“حلاقة كاملة”],barberId:“b10”,barberName:“أبو عمر”,date:“2025-04-25”,time:“10:00”,total:70,status:“approved”}],rating:4.7,status:“approved”},
];
...
const[search,setSearch]=useState(””);
const[adminPw,setAdminPw]=useState(ADMIN_PWD);
...
onKeyDown={e=>e.key===“Enter”&&(pw===adminPw?onEnter():setErr(true))}/>
{err&&<div style={{color:”#e74c3c”,fontSize:12,marginBottom:8}}>❌ كلمة المرور غير صحيحة</div>}
<button style={{…G.sub,marginTop:4}} onClick={()=>pw===adminPw?onEnter():setErr(true)}>دخول</button>
...
// بحث شامل
const q=search.trim().toLowerCase();
const filterSalons=list=>!q?list:list.filter(s=>[s.name,s.owner,s.phone,s.ownerPhone,s.region,s.gov,s.center,s.village,s.address].join(” “).toLowerCase().includes(q));
...
const bx={background:”#13131f”,borderRadius:13,padding:“13px 10px”,textAlign:“center”,border:“1px solid #2a2a3a”,position:“relative”,overflow:“hidden”};
...
```
  ...
  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
    {[
      ...
      {n:totalRev,        l:"إجمالي الإيرادات", c:"#f0c040", i:"💰", suffix:" ر"},
      {n:todayRev,        l:"إيرادات اليوم",    c:"#34d399", i:"💵", suffix:" ر"},
    ].map(({n,l,c,i,suffix})=>(
      <div key={l} style={{...bx,border:`1px solid ${c}33`}}>
        <div style={{position:"absolute",top:8,right:10,fontSize:20,opacity:.12}}>{i}</div>
        <div style={{fontSize:22,fontWeight:900,color:c,marginBottom:2}}>{n}{suffix||""}</div>
  ...
  {search&&<div style={{fontSize:11,color:"var(--p)",marginBottom:8,textAlign:"center"}}>نتائج البحث عن: "{search}"</div>}
    ...
    filteredBookings.length===0
      ...
      :<div style={{display:"flex",flexDirection:"column",gap:8}}>
        {filteredBookings.map((b,i)=>{
          const stColor=b.status==="approved"?"#27ae60":b.status==="rejected"?"#e74c3c":"var(--p)";
          const stLabel=b.status==="approved"?"✅ مقبول":b.status==="rejected"?"❌ مرفوض":"⏳ انتظار";
          return(
            <div key={i} style={{...G.bItem,borderRight:`3px solid ${stColor}`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                ...
                <div style={{textAlign:"left",flexShrink:0}}>
                  ...
                  <div style={{fontSize:10,color:stColor,marginTop:3}}>{stLabel}</div>
                </div>
              </div>
            </div>
          );
    ...
    filteredCustomers.length===0
      ...
      :<div style={{display:"flex",flexDirection:"column",gap:8}}>
        {filteredCustomers.map(c=>(
          <div key={c.id} style={{...G.bItem,borderRight:"3px solid #6aadff"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
  ...
  {tab==="password"&&(
    <div style={{background:"#13131f",borderRadius:13,padding:16,border:"1px solid #2a2a3a"}}>
      ...
      {[{l:"كلمة المرور الحالية",k:"old"},{l:"كلمة المرور الجديدة",k:"n1"},{l:"تأكيد كلمة المرور الجديدة",k:"n2"}].map(({l,k})=>(
        <div key={k} style={{marginBottom:10}}>
          <label style={{display:"block",fontSize:11,color:"#aaa",marginBottom:4,fontWeight:600}}>{l}</label>
          <input type="password" style={fi(pwForm.err&&k==="old")} value={pwForm[k]}
            onChange={e=>setPwForm(p=>({...p,[k]:e.target.value,err:""}))}/>
        </div>
      ))}
      {pwForm.err&&<div style={{color:"#e74c3c",fontSize:12,marginBottom:8}}>❌ {pwForm.err}</div>}
      <button style={G.sub} onClick={()=>{
        if(pwForm.old!==adminPw){setPwForm(p=>({...p,err:"كلمة المرور الحالية غير صحيحة"}));return;}
        if(pwForm.n1.length<4){setPwForm(p=>({...p,err:"كلمة المرور الجديدة قصيرة جداً"}));return;}
        if(pwForm.n1!==pwForm.n2){setPwForm(p=>({...p,err:"كلمتا المرور غير متطابقتين"}));return;}
        setAdminPw(pwForm.n1);
        setPwForm({old:"",n1:"",n2:"",err:""});
        toast$&&toast$("✅ تم تغيير كلمة المرور");
      }}>💾 حفظ كلمة المرور الجديدة</button>
      <div style={{fontSize:11,color:"#555",marginTop:8}}>⚠️ ملاحظة: كلمة المرور تُحفظ في الجلسة الحالية فقط</div>
    </div>
  )}
...
setSalons(p=>p.map(s=>s.id===salon.id?{…s,
name:f.name,phone:f.phone,address:f.address,locationUrl:f.locationUrl,
shiftEnabled:f.shiftEnabled,
shift1Start:f.shift1Start,shift1End:f.shift1End,
shift2Start:f.shift2Start,shift2End:f.shift2End,
workStart:f.workStart,workEnd:f.workEnd,
services:f.services,prices:f.prices,
barbers:f.barbers,tone:f.tone,
}:s));
...
const SECS=[
{id:“info”,icon:“📋”,label:“المعلومات”},
{id:“hours”,icon:“🕐”,label:“الأوقات”},
{id:“services”,icon:“✂”,label:“الخدمات”},
{id:“barbers”,icon:“💈”,label:“الحلاقون”},
{id:“tone”,icon:“🔔”,label:“النغمة”},
];
...
style={{flexShrink:0,padding:“6px 10px”,borderRadius:9,border:`1.5px solid ${sec===s.id?"var(--p)":"#2a2a3a"}`,background:sec===s.id?“var(–pa12)”:”#1a1a2e”,color:sec===s.id?“var(–p)”:”#888”,cursor:“pointer”,fontSize:11,fontFamily:”‘Cairo’,sans-serif”,fontWeight:sec===s.id?700:400,display:“flex”,alignItems:“center”,gap:4}}>
{s.icon} {s.label}
</button>
))}
</div>
...
```
  ...
  {sec==="hours"&&<div style={box}>
    ...
    {!f.shiftEnabled
      ...
      :<>
        <div style={{fontSize:11,color:"#888",marginBottom:6}}>⛅ الوردية الصباحية</div>
        ...
        <div style={{fontSize:11,color:"#888",marginBottom:6}}>🌙 الوردية المسائية</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
  ...
  {sec==="services"&&<div style={box}>
    ...
    {f.services.map(svc=>(
      <div key={svc} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,background:"#0d0d1a",borderRadius:9,padding:"8px 10px",border:"1px solid #2a2a3a"}}>
        ...
        <input type="number" min="0"
          style={{width:75,padding:"6px 8px",borderRadius:7,border:"1.5px solid #2a2a3a",background:"#13131f",color:"var(--p)",fontSize:13,fontFamily:"inherit",outline:"none",textAlign:"center",direction:"ltr"}}
          value={f.prices[svc]||0}
          onChange={e=>setF(p=>({...p,prices:{...p.prices,[svc]:+e.target.value}}))}/>
    ...
    <div style={{marginTop:12}}>
      <div style={{fontSize:11,color:"#666",marginBottom:6}}>خدمات جاهزة — اضغط لإضافة:</div>
  ...
  {sec==="barbers"&&<div style={box}>
    ...
    {f.barbers.length===0&&<div style={G.empty}>لا يوجد حلاقون — أضف أول حلاق</div>}
    {f.barbers.map((b,i)=>(
      <div key={b.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,background:"#0d0d1a",borderRadius:9,padding:"9px 11px",border:"1px solid #2a2a3a"}}>
        ...
        <input style={{...inp,flex:1,padding:"6px 10px"}} value={b.name}
          onChange={e=>setF(p=>({...p,barbers:p.barbers.map((x,j)=>j===i?{...x,name:e.target.value}:x)}))}/>
  ...
  {sec==="tone"&&<div style={box}>
    <div style={hdr}>🔔 نغمة التنبيه عند الحجز</div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
      {TONES.map(t=>{
        const active=f.tone===t.id;
        return(
          <button key={t.id} onClick={()=>{upd("tone",t.id);playTone(t.id,0.8);}}
            style={{padding:"11px 8px",borderRadius:10,border:`1.5px solid ${active?"var(--p)":"#2a2a3a"}`,background:active?"var(--pa12)":"#1a1a2e",color:active?"var(--p)":"#aaa",cursor:"pointer",fontSize:12,fontFamily:"'Cairo',sans-serif",fontWeight:active?700:400,textAlign:"center"}}>
            {t.label} {active&&"✓"}
          </button>
        );
      })}
    </div>
    <div style={{fontSize:11,color:"#555",marginTop:8}}>اضغط على النغمة لمعاينتها</div>
  </div>}
...
return(
<div style={G.page}><div style={G.fp}>
<div style={G.fh}><button style={G.bb} onClick={()=>setView(“home”)}>→</button><h2 style={{...G.ft,flex:1}}>مرحباً، {customer.name} 👋</h2><button style={{…G.delBtn,border:“1.5px solid #888”,color:”#aaa”,background:“transparent”}} onClick={()=>{setCustomerSession(null);setView(“home”);}}>خروج</button></div>
<div style={G.tabRow}>
...
<div style={{display:“flex”,justifyContent:“space-between”,alignItems:“center”}}>
<div><div style={{fontSize:14,fontWeight:700,color:”#fff”}}>✂ {s.name}</div><div style={{fontSize:11,color:”#888”}}>📍 {s.gov||s.region}{s.village?` · ${s.village}`:””}</div></div>
...
return <div key={i} style={{…G.bItem,borderRight:`3px solid ${h.rating?"var(--p)":"#2a2a3a"}`}}>
<div style={{display:“flex”,justifyContent:“space-between”,alignItems:“flex-start”}}>
<div>
...
<div style={{fontSize:11,color:“var(–p)”}}>💰 {h.total||0} ريال</div>
</div>
{h.rating>0&&<div style={{display:“flex”,gap:1}}>
{[1,2,3,4,5].map(n=><span key={n} style={{fontSize:14,color:n<=h.rating?”#f0c040”:”#333”}}>★</span>)}
</div>}
</div>
<InlineStarRating
rated={h.rating||0}
