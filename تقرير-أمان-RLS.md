...
**التالي:** المرحلة 2 — تصميم واختبار سياسات RLS الفعلية (`auth.uid()` بدل `true`) على DORK-TEST أولاً، ثم DORK.
...
**تقييم الخطورة:** 🔴 عالية — تسريب بيانات شخصية حقيقية (اسم + جوال) لعملاء حقيقيين، قائم الآن على الإنتاج (DORK)، ومكتشَف بالصدفة أثناء تحضير Tier 2 وليس نتيجة فحص أمني منهجي مخصّص.
...
**✅ تطبيق على DORK-TEST (نفس اليوم، بعد تأكد استقرار DORK):** قارن أحمد قائمة أعمدة `customers` بين المشروعين — مطابقة 100% (20 عمود). شُغّل نفس سكربت الإصلاح (REVOKE جدول + GRANT عمودي) على DORK-TEST، وتحقّق `table_privileges` بعده أكّد نفس النتيجة النظيفة (بس TRUNCATE/REFERENCES/TRIGGER، صفر SELECT/UPDATE). **DORK وDORK-TEST متطابقان تماماً على هذا الإصلاح الآن.**
...
**⚠️ اكتُشف أثناء هذا الإعداد: كان فيه محاولة سابقة غير مكتملة لإعداد SMTP** (Username محفوظ خطأً كـ"DORK-TEST" بدل "resend"، Host فاضي) — من جلسة سابقة، ما وصلت لنتيجة، ولا حد لاحظ إنها ناقصة لين الليلة.
