...
import { getCustomerClassification } from "../utils/notifications";

const G = {
  empty: { textAlign: "center", color: "#666", padding: "20px 10px", fontSize: 12 },
  bItem: { background: "rgba(255,255,255,.03)", borderRadius: 8, padding: 12, marginBottom: 8 },
  xBtn: { background: "transparent", border: "none", color: "#e74c3c", cursor: "pointer", fontSize: 12, fontWeight: 700, padding: "4px 8px" },
  accBtn: { fontSize: 11, padding: "4px 12px", borderRadius: 8, border: "1px solid #27ae60", background: "transparent", color: "#27ae60", cursor: "pointer", fontFamily: "inherit", fontWeight: 700 },
  rejBtn: { fontSize: 11, padding: "4px 12px", borderRadius: 8, border: "1px solid #e74c3c", background: "transparent", color: "#e74c3c", cursor: "pointer", fontFamily: "inherit", fontWeight: 700 },
};

function getSlotsForSalon(salon) {
  if (!salon?.workHours) return [];
  const slots = [];
  const { startTime, endTime, slotDuration } = salon.workHours;
              ...
              {showWaiting&&waitingList.map((w,idx)=>{
                ...
                <div key={w.id} style={{padding:"8px 0",borderTop:"1px solid #2a2a3a"}}>
                  ...
                  <div style={{display:"flex",gap:6,marginTop:6}}>
                    {w.slotTime&&<button style={G.accBtn} onClick={()=>acceptFromWaiting(w)}>✅ قبول للموعد</button>}
                    <button style={G.rejBtn} onClick={()=>rejectFromWaiting(w)}>❌ رفض</button>
                  </div>
                </div>
              )})}
            </div>
