...
export function toAppCustomer(row) {
  ...
  if (!history.length) {
    try {
      const h = localStorage.getItem(`hist_${row.id}`);
  ...
  if (!favs.length) {
    try {
      const f = localStorage.getItem(`favs_${row.id}`);
  ...
  return {
    ...
    reviewSalon: row.review_salon || null,
...
export function getCustomerClassification(customer) {
  ...
  const total = customer.totalBooks || 0;
  const spent = customer.totalSpent || 0;

  if (total >= 20 || spent >= 2000) return "vip";
  if (total >= 10 || spent >= 1000) return "loyal";
  if (total >= 5 || spent >= 500) return "regular";
