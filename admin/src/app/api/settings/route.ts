...
export async function GET() {
  ...
  const { data: appRows } = await sb.from("app_settings").select("social_links,ui_settings").order("id", { ascending: true }).limit(1);
  ...
  return NextResponse.json({ social, appearance, pinned, week_salon, auto_approve_salons });
}
...
export async function PATCH(req: NextRequest) {
  ...
  // تحديث social و appearance في جدول app_settings الموجود
  if ("social" in body || "appearance" in body) {
