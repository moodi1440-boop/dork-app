...
> هذه القائمة مرجعية فقط للملف الحالي (`App.jsx`). لوحة الإدارة (`admin/`) فيها 78 رمز فريد لم يبدأ العمل عليها بعد — راجع `bug-list.md` بند 47 لتفاصيل سجل القرارات الكامل لكل أيقونة (سبب التصميم، الخيارات المرفوضة، تاريخ كل دفعة).
